...
-- 
SELECT
	user_name, (SELECT AVG(user_height) FROM user_tbl) AS avg_height
FROM
	user_tbl;

-- FROM절 SUB QUERY를 활용한 범위좁히기 개념에 대해서 보겠습니다.
-- FROM절 다음에는 테이블명만 올 수 있는게 아니고, 데이터 시트형식을 갖춘 자료라면
-- 모든 형태의 자료를 적어줄수 있습니다.
-- user_tbl 중 키가 170 미만인 요소만 1차적으로 조회한 결과시트를 토대로 전체데이터 조회구문
...
-- 서브퀴르의 실제 활용 : 최지선보다 키가 큰 사람을 한 줄로 결과 얻기
-- 기존방식으로 처리하는 경우
...
FROM
	user_tbl
WHERE
	user_height > (SELECT user_height FROM user_tbl WHERE user_name = '최지선');
    
-- 위코드를 참조해서 평균보다 체중이 낮은 사람만 조회하는 퀴리문을 서브쿼리를 활용해 
...
FROM
	user_tbl
WHERE
	user_weight < (SELECT AVG(user_weight) FROM user_tbl);

-- 지역이 경기인 사람들 중 키가 제일 큰 사람보다 키가 더 큰사람을 조회해주세요
...
WHERE
	user_address
IN 			  -- user_address 가 속하는 
	(SELECT
		user_address
	 FROM
		user_tbl -- 키가 큰 지역
	 WHERE
		user_height > (SELECT AVG(user_height) FROM user_tbl) -- 전체키평균보다
GROUP BY
	user_address);
