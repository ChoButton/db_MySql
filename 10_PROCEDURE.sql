...
BEGIN
	DECLARE var1 INT; # 프로시저내 주석은 #으로 답니다. DECLARE는 변수 선언문
    SET var1 = 100; # 위에 var1 INT로 선언된 변수에 100대입
...
-- 평균보다 키 큰 사람 전체 목록을 호출해주시는 프로시저입니다.
...
BEGIN
	SELECT * FROM
		user_tbl
	WHERE
		user_height > (SELECT AVG(user_height) FROM user_tbl);
END $$
DELIMITER ;
...
-- 프로시저를 삭제시 DROP PROCEDURE 프로시저명; -- ()는 쓰지 않습니다.
-- 특정 프로시는 DB에 소속된것이기 때문에 반드시 DB가 먼저 선택되어야 명령을 수행합니다.
...
BEGIN
    ...
    DECLARE days INT; # 경과일수 저장 변수

	SELECT entry_date INTO entryDate #쿼리문의 결과로 나온 값을 entryDate 변수에 대입
		FROM user_tbl WHERE user_num = 2;
	
    SET todayDate = CURRENT_DATE(); # 오늘날짜를 얻어오는 내장함수, 오늘 날짜만 얻어옴 now() 시, 분, 초 까지 얻어옴
...
*/
DELIMITER $$
CREATE PROCEDURE getThree(IN user_number INT)
BEGIN
	DECLARE entryDate DATE; # 날짜 판독을 위한 저장변수 선언
    DECLARE todayDate DATE; # 오늘 날짜 저장변수 선언
