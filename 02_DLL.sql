/* 좌측의 SCHEMAS의 DATABASE는 더블클릭이나 우클릭으로 호출 가능하지만
use DB명; 을 수행해 호출할수도 있습니다.*/

-- Workbench(윈도우)에서 수행가능한 구문은 거의 모드 CLI환경에서 수행 가능합니다.
use bitcamp06;
...
CREATE TABLE user_tbl(
    ...
    user_height int, -- 자리수 제한없음
    entry_date date -- 회원 가입일
);

/* 특정 테이블은 원래 조회할 때 SELECT * FROM 데이터베이스명.테이블명;
형식으로 조회해야 합니다.
그러나 use 구문 등을 이용해 데이터베이스를 지정한 경우는 데이터베이스를 생략할수 있습니다.*/
...
-- 주의! WGHERE를 걸지 않으면 해당 컬럼의 모든 값을 다 통일시켜 버립니다.
...
-- 테이블이 존재하지 않다면 삭제구문을 실행하지 않아 에러를 발생시키지 않음
 ...
 -- 만약 WHERE없이 삭제시 truncate 처럼 작동합니다.
...
INSERT INTO user_tbl
	VALUE (null, '강개발', 1994, '경남', 178, '2020-08-02'),
		  (null, '최지선', 1998, '전북', 170, '2020-08-03'),
          (null, '류가연', 2000, '전남', 158, '2022-08-20');
SELECT * FROM user_tbl;

-- INSERT ~ SELECT 를 이용한 데이터 삽입을 위해 user_tbl과  동일한 테이블을 하나더 만듭니다.
...
-- user_tb2에 user_tbl의 자료 중 생년 1995년 이후인 사람의 자료만 복사해서 삽입하기
...
-- 어떤 유저는 반드시 user_tbl에 존재하는 유저만 추가할수 있습니다.
...
DELETE FROM buy_tbl WHERE buy_num = 7;
