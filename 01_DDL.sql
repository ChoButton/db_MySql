...
만약 모든 칼럼에 값을 넣는 다면 위 구문엣허 테이블명 다음 오는
컬럼명을 생략할수 있음
*/
...
INSERT INTO users VALUES (2, 'abc3456', '마바사', 'abc@ab.com');
 ...
 /* 문제
 3번 유저를 임의로 추가해주세요
 조회구문은 이메일 회원번호 아이디 순으로 세 컬러만 조회하는 구문*/
 
 INSERT INTO users VALUES(3, 'abc789', '조승연', '조승연@조.com');
 ...
 -- ALTER TABLE 테이블명 ADD(추가할컬럼명 자료타입(크기));
 ALTER TABLE users ADD (u_address VARCHAR(30));
 
 SELECT * FROM users;
 
 -- users 이메일 컬럼을 삭제해 주세요
 ALTER TABLE users DROP email;
 
 -- users address 에 유니크 제약조건 별칭 부여해서 걸기
 ALTER TABLE users ADD CONSTRAINT u_address_unique UNIQUE (u_address);
 
 -- INSERT INTO 구문으로 7, 8번 유저를 추가해주시되
 -- 주소는 둘 다 '강남구'로 넣으려고 시도해 보세요
 INSERT INTO users VALUES(7, 'aaa', '강남이', '강남구');
 INSERT INTO users VALUES(8, 'bbb', '강북이', '강남구');
 
 -- 주소에 걸린 UNIQUE 제약을 없애고 8번을 마저 추가해주세요
 -- 위에서 u_address 컬럼에 걸린 제약조건의 별칭인 u_address_unique를 삭제
 ALTER TABLE users DROP CONSTRAINT u_address_unique;
 
 -- users테이블명을 members로 바꾸겠습니다.
 RENAME TABLE users TO members;
 
 -- TRUNCATE TABLE 은 내부 데이터만 소각합니다. 빈테이블은 남아있습니다.
